...
description: "무료 블로그 플랫폼을 비교하고, 개발자가 GitHub Pages를 선택해야 하는 이유를 정리한다."
---

# 왜 GitHub Pages인가 — 무료 블로그 호스팅의 선택지

> 블로그를 만들고 싶지만 호스팅 비용은 들이고 싶지 않은 개발자를 위한 선택 가이드.

<!-- more -->

---

이 글은 **무료로 기술 블로그 만들기** 시리즈의 첫 번째 편이다.

| 편 | 제목 |
|---|------|
| **1** | **왜 GitHub Pages인가 — 무료 블로그 호스팅의 선택지** |
...
기술 블로그를 시작하려면 가장 먼저 "어디에 글을 올릴 것인가"를 정해야 한다. 선택지는 크게 네 가지로 나뉜다.
...
- 개발자에게 마크다운 중심 워크플로우가 아니다. 에디터가 WYSIWYG 기반이라 코드 중심 글 작성에 불편하다.
...
- 완전 무료다. 호스팅 비용이 0원이다.
- GitHub 저장소가 곧 블로그다. 버전 관리, 이슈 트래킹, PR 기반 협업이 자연스럽다.
- 마크다운으로 글을 쓴다. 개발자에게 가장 익숙한 포맷이다.
- 글이 곧 Git 히스토리다. 언제 뭘 수정했는지 추적할 수 있다.
...
**2. 개발자의 워크플로우에 자연스럽게 녹아든다.** 글을 마크다운으로 쓰고, Git으로 관리하고, PR로 리뷰하고, push하면 배포된다. 별도의 관리 도구를 익힐 필요가 없다.
...
- **기술 문서 작성에 최적화되어 있다.** 코드 하이라이팅, 탭, admonition, 검색 등 개발 블로그에 필요한 기능이 기본으로 들어 있다.
